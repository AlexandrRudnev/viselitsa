...
,Каннинг,Кольцов,Конфуций,Ларошфуко,Лессинг,Ломоносов,Менандр,Мятлев,Некрасов,Овидий,
Панфилов,абрикос,баталия,болт,гарлем,гарнитур,горал,дивалин,каретта,катрен,колпак,пальто,
парад,ракета,рамка,сила,сурок,тальник,ундина,ухта,яшма`,
...
сражавшейся в Московской битве)_Если из этого кустарника семейства розовых убрать две буквы, 
то получается очертания предмета._Если из этого военного действия убрать две буквы, то получится 
часть туловища_Если из этой детали убрать одну букву, то получится река в Румынии_Если из этого 
района всем известного города убрать одну букву, то получится женское помещение_Если из этого 
набора предметов убрать две буквы, то получится отличное добавление к мясным блюдам_Если из этого 
парнокопытного животного убрать одну букву, то получится крыло Конвента в период Великой французкой
 революции, представлявшее якобинцев_Если из этого мифического карлика убрать одну букву, то 
 получится аминокислота_Если из этой морской черепахи убрать одну букву, то она сможет пригодиться 
 Золушке_Если из этого четверостишия убрать одну букву, то получится представитель народа, живущего 
 в Мьянме и на западе Тайланда_Если из этого предмета одежды убрать две буквы, то получится высший
  сорт фаянса_Если убрать из этого предмета одежды две буквы, то получится музыкальный смычковый 
  инструмент_Если убрать из этого праздничного действия одну букву, то получится упряжка_Если из 
  этого аппарата убрать две буквы, то получится вкусная рыба_Если из этого украшения страницы убрать
  одну букву, то получится большой ларец для хранения мощей святых_Если из этого физического понятия
  вычесть одну букву, то получится божество, олицетворения разума, познания, мудрости (в египетской 
мифологии)_Если из этого зверя убрать одну букву, то получится промежуток времени_Если из этого 
кустарника убрать две буквы, то получится светлый мягкий слоистый минерал_Если из этого 
сказочного существа убрать две буквы, то получится единица измерения (единица силы)_Если из 
этого города убрать одну букву, то получится вкусная еда активного туриста_Если из той горной 
 породы убрать одну букву, то получится бог, правитель царства мёртвых
 (вбрахманизме и индуизме)`;
